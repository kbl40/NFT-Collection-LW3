...
export default function Home() {
  ...
  /**
   * presaleMint: Mint an NFT during the presale phase
   */
  ...
  const presaleMint = async() => {
    try{
      ...
      const whitelistContract = new Contract(NFT_CONTRACT_ADDRESS, contractAbi.abi, signer);
  ...
  const startPresale = async () => {
    try {
      ...
      // Call the startPresale function from the contract
  ...
  /**
   * checkIfPresaleStarted: checks if the presale has started by quering the 
   * 'presaleStarted' variable in the contract
   */
  const checkIfPresaleStarted = async () => {
    try {
      ...
      const nftContract = new Contract(NFT_CONTRACT_ADDRESS, contractAbi.abi, provider);

      // Call the presaleStarted function from the contract
  ...
  /**
   * chechIfPresaleEnded: checks if the presale has ended by quering the 'presaleEnded' state variable
   */
  const checkIfPresaleEnded = async () => {
    try {
      ...
      // _presaleEnded is a Big Number, so we are using the lt() instead of '<'
      // Date.now()/1000 returns the current time in seconds
      // We compare if the _presaleEnded timestamp is less than the current time which means the presale has ended
  ...
  const getOwner = async () => {
    try {
      // Get the provider from web3Modal, which in our case is MetaMask
      // No need for the signer here, as we are only reading from the contract
      const provider = await getProviderOrSigner();
  ...
  const getTokenIdsMinted = async () => {
    try {
      // Get the provider from web3Modal, which in our case is MetaMask
      // No need for the Signer as we are only reading state
      const provider = await getProviderOrSigner();
  ...
  // useEffects are used to react to changes in the state of the website (dapp)
